...
전역 NPC 340~480개는 아직 실제 산출물이 아니라 이 프롬프트 단계다.
```

현재 부족한 것:

```text
1. E02~E12 전역 NPC/잔향/기록 씨앗.
2. 96개 정식 캠페인별 최소 스토리 담당자.
3. 미스터리/반전/작은 결말/해금 단서의 전역 배치.
4. 플레이어블 20인과 NPC 씨앗의 연결 매트릭스.
5. 어떤 권역에서 어떤 진실이 먼저 보이고, 어떤 진실은 늦게 열리는지에 대한 노출 잠금.
```
...
여기서 실제 NPC 340~480개를 작성하지 말고, 제작팀에게 이 프롬프트를 전달해 산출물을 만들게 한다.
