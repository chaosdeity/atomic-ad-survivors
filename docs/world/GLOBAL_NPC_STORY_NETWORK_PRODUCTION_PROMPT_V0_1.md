...
후속 전역 수량 만족 기준은 `768개 제작 슬롯`이며, 확장 슬롯 레지스트리에서 별도로 충족했다.
...
1차 실제 산출물 5개, 전역 NPC 이름/원점 검수, 수량 충족 매트릭스, 확장 슬롯 레지스트리는 작성 완료되었다.
이후에는 768개 제작 슬롯을 기준으로 전역 NPC 이름/원점 검수를 계속 보강한다.
