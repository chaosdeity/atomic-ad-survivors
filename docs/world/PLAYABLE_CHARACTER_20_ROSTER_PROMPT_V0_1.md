...
- 3번 OPEN-HOST, 4번 퇴원 보류 환자, 5번 반송 수취인, 6번 폐충전소 계량자, 7번 침묵권 피난자로 둔다.
- 4번 퇴원 보류 환자 상세 결재는 조건부 통과했다. 힐러화 방지와 R02-L01~L03 해금 범위 보완이 필요하다.
- 4번 퇴원 보류 환자 보완 문서도 완료되었다. 통과로 승격 가능하며, 다음은 프로필 v1.0이다.
...
- 4번은 퇴원 보류 환자, 5번은 반송 수취인, 6번은 폐충전소 계량자, 7번은 침묵권 피난자로 둔다.
- 4번 퇴원 보류 환자 상세 결재는 조건부 통과로 완료되었다.
- 4번 퇴원 보류 환자 보완 문서는 통과로 승격 가능 판정으로 완료되었다.
- 리오는 8번 후보 슬롯으로 둔다.
- 3~7번에는 새 로봇을 넣지 않는다.
- 9~20번은 이름을 확정하지 않아도 된다.
- 각 슬롯은 아래 표만 작성한다.
...
다음 방에서는 20명을 다시 늘어놓지 않는다. PATCH는 2번으로 잠겼고 3~7번 후보 선정, 4번 퇴원 보류 환자 상세 결재, 보완 문서도 완료되었으므로, 퇴원 보류 환자 프로필 v1.0만 다룬다.
