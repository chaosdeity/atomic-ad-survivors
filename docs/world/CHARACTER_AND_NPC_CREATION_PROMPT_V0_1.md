...
보급소/NPC 기준:
40. story/02_hub/silence_outpost.md
41. story/02_hub/npcs.md
42. story/02_hub/mina_npc_profile_v1_0.md
43. story/02_hub/mina_npc_drama_draft_v0_1.md
44. story/05_progression/run_story_hooks.md
45. story/05_progression/story_trigger_schema.md
...
3. 윤서는 1번 슬롯으로 잠근다.
4. PATCH는 2번 조건부 후보 슬롯으로 둔다.
5. OPEN-HOST는 R01 NPC 선등장 후 플레이어블 전환 후보로 둔다.
6. 리오는 8번 후보 슬롯으로 잠근다.
7. 3~7번은 E01/R01/R02/R03/R04/R08 안에서 찾아야 한다.
8. 로스터 중복 검사를 한다.
9. 다음 작업은 PATCH가 2번째 후보로 충분한지 별도 판정하는 것이다.
10. 그 다음 3~7번 초반 플레이어블 후보를 선정한다.
11. NPC는 플레이어블 로스터와 별도로 만든다.
12. NPC도 이름만 나열하지 말고, 시설/상태 반응/잘못 믿는 것/플레이어블이 아닌 이유를 붙인다.
...
PATCH가 조건부를 넘지 못하면 2번째 후보는 OPEN-HOST 계열로 재검토한다.
...
다음에는 PATCH 2번 플레이어블 최종 판정을 하라.
그 뒤 3~7번 초반 플레이어블 후보를 선정하라.
...
```text
윤서 1번, PATCH 2번 조건부 후보, 리오 8번을 잠그고,
OPEN-HOST를 R01 NPC 선등장 후 플레이어블 전환 후보로 반영하며,
3~7번 초반 후보의 기능과 중복 위험을 먼저 보여준다.
...
현재 기준에서는 `story/06_characters/playable_20_roster_blueprint_v0_1.md`를 먼저 보고, 20명을 다시 나열하지 않는다. 다음 작업은 **PATCH 2번 플레이어블 최종 판정**이다.
