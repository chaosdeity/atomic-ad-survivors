...
const ROLE_STATS := {
	"basic": {"hp": 1.0, "speed": 1.0, "radius": 8.0, "contact": 1.0, "sprite": "billboard", "defense": DamageRouter.DEFENSE_TYPE_NORMAL},
	"fast": {"hp": 0.80, "speed": 1.55, "radius": 7.0, "contact": 0.82, "sprite": "coupon", "defense": DamageRouter.DEFENSE_TYPE_ANTI_CHARGE},
	"tank": {"hp": 2.85, "speed": 0.62, "radius": 13.0, "contact": 1.35, "sprite": "appliance", "defense": DamageRouter.DEFENSE_TYPE_ANTI_AUTO},
	"speaker": {"hp": 1.35, "speed": 0.72, "radius": 10.0, "contact": 0.92, "sprite": "billboard", "defense": DamageRouter.DEFENSE_TYPE_NORMAL},
	"charger": {"hp": 1.12, "speed": 1.05, "radius": 9.0, "contact": 0.95, "sprite": "coupon", "defense": DamageRouter.DEFENSE_TYPE_ANTI_CHARGE},
	"signal": {"hp": 1.65, "speed": 0.82, "radius": 11.0, "contact": 1.15, "sprite": "billboard", "defense": DamageRouter.DEFENSE_TYPE_EXPOSED_CORE},
}
const ELITE_DEFENSE_TYPE := DamageRouter.DEFENSE_TYPE_PLATED
...
func spawn_enemy(elapsed: float, player_pos: Vector2, rng: RandomNumberGenerator, wave_params: Dictionary = {}) -> void:
	...
	enemies.append({
		...
		"elite": elite,
		"role": role,
		"defense_type": ELITE_DEFENSE_TYPE if elite else String(role_stats.get("defense", DamageRouter.DEFENSE_TYPE_NORMAL)),
...
func damage_enemies_in_radius(center: Vector2, radius: float, damage: float, player_stats: Dictionary, max_targets: int = -1, damage_type: String = DamageRouter.DAMAGE_TYPE_BURST) -> int:
	return damage_enemies_in_radius_with_hits(center, radius, damage, player_stats, max_targets, damage_type).size()

func damage_enemies_in_radius_with_hits(center: Vector2, radius: float, damage: float, player_stats: Dictionary, max_targets: int = -1, damage_type: String = DamageRouter.DAMAGE_TYPE_BURST) -> Array[Dictionary]:
	...
	for i in range(enemies.size()):
		if center.distance_squared_to(enemies[i]["pos"]) > radius_sq:
			continue
		var result := apply_damage(i, damage, damage_type, player_stats)
		if result.is_empty():
			continue
...
func apply_damage(index: int, base_damage: float, damage_type: String, player_stats: Dictionary) -> Dictionary:
	if index < 0 or index >= enemies.size():
		return {}
	var enemy := enemies[index]
	var dmg_result := DamageRouter.calculate_damage({
		"target_type": DamageRouter.TARGET_ENEMY,
	...
	var final_damage := float(dmg_result["final_damage"])
	enemy["hp"] = float(enemy["hp"]) - final_damage
	mark_hit(index)

	var hit := dmg_result.duplicate()
	hit["index"] = index
	hit["pos"] = Vector2(enemy["pos"])
	hit["damage"] = final_damage
	hit["defense_type"] = String(enemy.get("defense_type", DamageRouter.DEFENSE_TYPE_NORMAL))
	return hit

func damage_enemy(index: int, damage: float, player_stats: Dictionary, damage_type: String = DamageRouter.DAMAGE_TYPE_AUTO) -> Array[Vector2]:
	apply_damage(index, damage, damage_type, player_stats)
	return cleanup_dead()
