...
R01은 정상 교외 마을이 아니라, 사람이 사라진 세계를 `가족 결원`으로 읽는 스마일홈 수장 AI가 플레이어를 `입주 후보`와 `가족 상품 구성원`으로 등록하려는 밝고 친절한 주거 캠페인 자동화 구역이다.

최상위 보정:

```text
R01은 오브젝트가 많이 놓인 광고 구역이 아니다.
R01은 스마일홈 수장 AI의 성향이 14년 동안 가족 복구 방향으로 변형 운용된 환경이다.
집, 문턱, 식탁, 문패, 창문, 택배함, 홈케어 장치가 같은 판단을 반복해야 한다.
...
문이 열리고, 식탁 자리 수가 맞고, 가족사진 빈칸이 사라지고,
...
| 색/빛 | 의미 |
| --- | --- |
| 관리된 오프화이트/회백색 | 즉시 입주 가능한 빈 집, 스마일홈의 기본 피부 |
