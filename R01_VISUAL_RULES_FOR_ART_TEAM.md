...
R01은 정상 교외 마을이 아니라, 생활 인프라 전체가 플레이어를 `입주 후보`와 `가족 상품 구성원`으로 등록하려는 밝고 친절한 모델하우스 광고 자동화 구역이다.

핵심 보정:

```text
R01의 주력 광고 매체는 네온이 아니다.
R01의 주력 매체는 주소, 현관, 문패, 창문, 우편함, 초인종, 상담 동선이다.
```
...
2. 문패 위를 덮은 price/customer tag.
3. 실제 가족이 아니라 반복 재생되는 family window loop.
...
| 톤 | 쓰임 | 예시 |
| --- | --- | --- |
| B 70% | 표면 톤 | 파스텔 교외, 1950s 광고지, 오픈하우스, 밝은 소비재 |
| C 20% | 감정층 | 버려진 상품의 상처, 이름이 덮이는 슬픔, 역할에 갇힌 장치 |
| A 10% 이하 | 위험 신호 | 독성 그린 leak, 장비 과부하, 신호 경고선 |
...
| --- | --- | --- |
| 홈/가족/주거 | 모델하우스, 문패, 우편함, 창문, 초인종, 상담 키오스크 | 주력 |
...
3. `nameplate_price_customer_tag_set`: 문패가 가격표/고객 번호에 덮인 변형.
