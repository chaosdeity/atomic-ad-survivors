...
현재 플레이어블팀은 14번 한국어 임시명 상세 결재로 이동하지 않는다. 기존 코드명/해금 순번 마이그레이션 판정과 14~20 글로벌 이름/원점 재검토는 완료되었고, MODEL-15는 E10_C05 개방 후 후보로만 보존한다. LOCKOUT-17은 캠페인 미개방으로 대기하며 기존 산출물은 기능 초안/보류 산출물로만 취급한다. NAME-18 상세 결재로 넘어가지 않는다. 출시 초기 기준은 윤서 단독 U5, PATCH U1~U4 보류, OPEN-HOST NPC 선등장, HOLD/RETURN/METER/QUIET 출시 후 보류다. 다음에는 출시 초기 플레이어블 포인터 보정 완료 검수만 진행한다.
...
```text
현재는 대사고 직후가 아니라 훨씬 먼 후인류 시대다.
광고 생태는 장기간 운용되며 생활 환경이 되었고,
캠페인의 현재 상태는 특정 최근 연수 하나가 아니라 사고 전 서비스 목적과 사고 후 지역 승인 캐시, 자동화 인프라, 회색 오염, 인간 접근/약탈 기록이 장기간 결합한 결과다.
```
...
### PATCH-02 legacy_code 보존 / 출시 초기 U1~U4 보류

PATCH-02는 보급소 로봇 축의 기능 산출물과 legacy_code로 보존한다. 다만 출시 초기 +1 확정은 아니며, 현재 운용은 U1~U4 보류다.
...
반영:
PATCH-02 legacy_code 보존. 출시 초기 +1 확정 아님.
```

PATCH가 통과했으므로:

```text
OPEN-HOST는 R01 NPC 선등장 후 플레이어블 전환 후보로 유지한다. 출시 초기 U5 전환은 금지한다.
...
3~7번 기존 후보 산출물 선정도 완료되었지만 출시 초기 해금 대상은 아니다.
...
9번 Mika Arakawa / GATE-09 상세 결재도 완료되었다.
9번 GATE-09 조건부 해소도 완료되었다.
9번 GATE-09 프로필 v1.0도 완료되었다.
10번 Nora Weiss / BROAD-10 상세 결재도 완료되었다.
10번 BROAD-10 조건부 해소도 완료되었다.
10번 BROAD-10 프로필 v1.0도 완료되었다.
11번 Mara Kincaid / CLAUSE-11 상세 결재도 완료되었다.
11번 CLAUSE-11 조건부 해소도 완료되었다.
11번 Mara Kincaid / CLAUSE-11 프로필 v1.0도 완료되었다.
12번 Rafaela Costa / LEDGER-12 상세 결재도 완료되었다.
12번 LEDGER-12 조건부 해소도 완료되었다.
12번 Rafaela Costa / LEDGER-12 프로필 v1.0도 완료되었다.
...
15번 Maya Chen / MODEL-15 상세 결재도 완료되었다 / 조건부 통과.
15번 MODEL-15 조건부 해소도 완료되었다 / 통과로 승격 가능.
15번 Maya Chen / MODEL-15는 E10_C05 개방 후 후보로 보존한다.
17번 Omar Al-Hadid / LOCKOUT-17 산출물은 기능 초안/보류 산출물로만 취급한다.
17번 LOCKOUT-17은 캠페인 미개방으로 대기한다.
Leila Haddad / NAME-18 상세 결재로 넘어가지 않는다.
다음 작업은 출시 초기 플레이어블 포인터 보정 완료 검수다.
```
...
```text
E01 R02/R03/R04/R08 보조 NPC 최소안 완료.
완료 문서:
...
잠금:
1번 윤서
2번 PATCH-02 legacy_code 보존 / 출시 초기 U1~U4 보류
3번 OPEN-HOST 후보 예약 / NPC 선등장 / 출시 초기 U5 전환 금지
4번 강하람 / HOLD-04 / 퇴원 보류자 / 출시 후 보류
5번 한이루 / RETURN-05 / 반송 수취인 / 출시 후 보류
6번 차유건 / METER-06 / 폐충전소 계량자 / 출시 후 보류
7번 백여울 / QUIET-07 / 침묵권 피난자 / 출시 후 보류
8번 리오 프로필 v1.0 기능 산출물 보존 / 중반 해금 보류
...
```text
PATCH-02 기능 산출물 보존 / 출시 초기 U1~U4 보류 판정 반영 완료.
완료 문서:
story/06_characters/patch_playable_2nd_slot_final_decision_v0_1.md

3~7번 기존 후보 산출물 선정 완료 / 출시 초기 해금 대상 아님.
완료 문서:
story/06_characters/playable_slots_3_to_7_candidate_selection_v0_1.md
...
10번 BROAD-10 조건부 해소 완료 / 통과로 승격 가능.
10번 Nora Weiss / BROAD-10 프로필 v1.0 작성 완료.
11번 Mara Kincaid / CLAUSE-11 상세 결재 완료 / 조건부 통과.
11번 CLAUSE-11 조건부 해소 완료 / 통과로 승격 가능.
...
13번 전태주 / GUARD-13 상세 결재 전 후보 재검토 완료 / 조건부 유지.
13번 전태주 / GUARD-13 상세 결재 완료 / 조건부 통과.
13번 전태주 / GUARD-13 조건부 해소 완료 / 통과로 승격 가능.
13번 전태주 / GUARD-13 프로필 v1.0 작성 완료.
15번 Maya Chen / MODEL-15 상세 결재 완료 / 조건부 통과.
15번 MODEL-15 조건부 해소 완료 / 통과로 승격 가능.
15번 Maya Chen / MODEL-15는 E10_C05 개방 후 후보로 보존.
17번 Omar Al-Hadid / LOCKOUT-17 산출물은 기능 초안/보류 산출물로만 취급.
17번 LOCKOUT-17은 캠페인 미개방으로 대기.
...
용도:
플레이어블 4~7번과 충돌하지 않도록 각 지역 보조 NPC를 3명 이하, 직접 대면 1명 이하로 제한한 제작 기준.

다음:
9번 후보 상세 결재 완료.
GATE-09 조건부 해소 완료.
GATE-09 프로필 v1.0 작성 완료.
10번 Nora Weiss / BROAD-10 상세 결재 완료 / 조건부 통과.
10번 BROAD-10 조건부 해소 완료 / 통과로 승격 가능.
10번 Nora Weiss / BROAD-10 프로필 v1.0 작성 완료.
11번 Mara Kincaid / CLAUSE-11 상세 결재 완료 / 조건부 통과.
...
```text
9번 Mika Arakawa / GATE-09 / 미카, 막차 환승 심사자 상세 결재 완료.
완료 문서:
story/06_characters/gate_09_playable_decision_v0_1.md
...
고정:
미카는 빠른 이동 캐릭터가 아니라,
막차/환승/개찰 심사가 사람을 잔류자로 고정하기 전의 짧은 틈을 전투화한 사람.
...
고정:
빠른 이동/역무원/개찰 도둑화 금지.
E02_C07-L01~L02 노출 제한.
HP 0은 개찰 미통과 / 막차 탑승 실패 / 플랫폼 잔류 처리 / 저주파 안내 반복.
...
```text
9번 GATE-09 프로필 v1.0 완료.
완료 문서:
story/06_characters/gate_09_profile_v1_0.md

판정:
통과 / 9번 플레이어블 후보 프로필 v1.0 기능 산출물 보존 / 캠페인 개방 후 해금

고정:
미카는 빠른 이동 캐릭터가 아니다.
핵심은 막차 환승 심사, 개찰 미통과, 플랫폼 잔류 처리다.
HP 0은 개찰 미통과와 플랫폼 잔류 처리 직전 긴급 인양이다.
해금은 E02_C07-L02 이후 중반부다.
...
```text
10번 Nora Weiss / BROAD-10 / 노라, 공영 안내 수신자 상세 결재 완료.
완료 문서:
...
핵심은 공식 수신 과잉, 청취 확인, 대피 문장 자동 승인, 반복 송출 동기화다.
E07_C05-L01~L02 범위로 제한한다.
...
고정:
방송인/아나운서/언론인/해커/서포터화 금지.
여울은 덜 들려서 늦게 구해지고, 노라는 너무 잘 들려서 빨리 등록된다.
E07_C05-L01~L02 노출 제한.
HP 0은 청취 확인 완료 / 대피 문장 자동 승인 / 공영 안내 문구 프로필 고정 / 반복 송출 동기화.
...
```text
10번 BROAD-10 프로필 v1.0 완료.
완료 문서:
story/06_characters/broad_10_profile_v1_0.md

판정:
통과 / 10번 플레이어블 후보 프로필 v1.0 기능 산출물 보존 / 캠페인 개방 후 해금

고정:
노라는 공영 안내가 청취 확인 완료와 대피 문장 승인자로 확정하기 전의 판정을 늦추는 플레이어블 후보다.
HP 0은 청취 확인 완료 / 대피 문장 자동 승인 / 공영 안내 문구 프로필 고정 / 반복 송출 동기화다.
독일어 원문 표기는 검수 보류다.
...
```text
11번 Mara Kincaid / CLAUSE-11 상세 결재 완료.
완료 문서:
...
고정:
마라는 법률가/상인/쿠폰 캐릭터가 아니라,
약관의 작은 글씨, 자동결제 승인, 환불 예외, 회원 등급 고정 판정을 늦추는 생존자다.
E09_C06 대형마트 멤버십권만 열고 E09 전체 소비권과 E10_C06 앱스토어 결제권은 선점하지 않는다.
...
고정:
법률가/상인/쿠폰 캐릭터화 금지.
E09_C06-L01~L02 노출 제한.
R06 회원 등급 브로커와 LEDGER-12 라파 분리.
HP 0은 자동결제 승인 / 예외 조항 폐쇄 직전 긴급 인양.
...
```text
11번 CLAUSE-11 프로필 v1.0 완료.
완료 문서:
story/06_characters/clause_11_profile_v1_0.md

판정:
통과 / 11번 플레이어블 후보 프로필 v1.0 기능 산출물 보존 / 캠페인 개방 후 해금

고정:
마라는 법률가/상인/계산원/쿠폰 캐릭터가 아니다.
약관 조항 / 자동결제 승인 / 환불 예외 / 회원 등급 고정 판정을 늦추는 플레이어블 후보다.
HP 0은 자동결제 승인 / 예외 조항 폐쇄 / 환불 불가 처리 / 회원 등급 고정 직전 긴급 인양이다.
해금은 E09_C06-L02 이후 중반부다.
...
```text
12번 Rafaela Costa / LEDGER-12 상세 결재 완료.
완료 문서:
...
고정:
라파는 상인/블랙마켓/돈벌이 캐릭터가 아니라,
태그를 돈처럼 돌리던 비공식 장부를 보급소 규칙과 미정산 책임으로 되접는 후보다.
E11_C03 항만시장 회수권만 직접 열고 E11_C06은 심화 단서로 둔다.
...
고정:
상인/블랙마켓/돈벌이/태그 교환 UI 금지.
미나는 보급소 내부 정산, 라파는 외부 장부의 미정산 책임으로 분리.
R06 회원 등급 브로커, CLAUSE-11 마라, 윤서/RETURN-05/R03 회수·반송 축과 분리.
HP 0은 거래 장부 이름 고정 / 미정산 태그 폐쇄 직전 긴급 인양.
...
고정:
Rafaela Costa / LEDGER-12 / 라파, 항만시장 태그 중개자 표기 유지.
핵심 문장은 "장부에 적었다고 해서, 사람이 정산된 것은 아니다."
해금은 GATE-09/BROAD-10/CLAUSE-11 이후, E11_C03-L01~L02 이후다.
E11_C06은 심화 단서이며, E12_C06과 E06_C06은 선점하지 않는다.
...
```text
13번 GUARD-13 상세 결재 전 후보 재검토 완료.
완료 문서:
...
고정:
보호자/보증인 등록 거부 축은 유지한다.
전태주 이름은 조건부 유지하되 확정하지 않는다.
원점은 E12_C02 이동진료 승인권 우선, E04_C02 열대격리 진료권 보조로 검토한다.
...
타인의 치료 접근권이 보호자/보증인 서명으로 담보 잡히기 전, 그 책임 칸을 비워 두는 후보다.
전태주 이름은 조건부 유지하며 프로필 v1.0 전 이름/국적/언어권 검수가 필요하다.
E12_C02 이동진료 승인권을 우선 원점으로 두고, E04_C02는 보조 후보로 둔다.
HP 0은 보호자 서명 확정 / 치료 접근권 담보 고정으로 차별화한다.
...
고정:
전태주 이름은 유지 가능하되 프로필 v1.0에서 최종 고정 검토한다.
전태주를 E12_C02 전체 현실권 대표명처럼 쓰지 않는다.
E12_C02 이동진료 승인권을 원점으로 잠근다.
E04_C02는 보조 단서, E01/R02 직접 원점은 보류로 둔다.
HOLD-04와 R02 의료 NPC 기능을 침범하지 않는다.
HP 0은 보호자 서명 확정 / 치료 접근권 담보 고정으로 잠근다.
...
```text
13번 GUARD-13 프로필 v1.0 완료.
완료 문서:
story/06_characters/guard_13_profile_v1_0.md

판정:
통과 / 13번 플레이어블 후보 프로필 v1.0 기능 산출물 보존 / 캠페인 개방 후 재검토

고정:
전태주 / GUARD-13 / 태주, 보호자 등록 거부자 표기는 유지한다.
전태주는 E12_C02 전체 현실권 대표 이름이 아니며 로컬라이즈 검수 보류다.
원점은 E12_C02 이동진료 승인권으로 잠근다.
E04_C02는 보조 단서, E01/R02 직접 원점은 보류로 둔다.
HOLD-04는 환자/퇴원 보류, GUARD-13은 보호자/보증인/치료 접근권 담보 절차로 분리한다.
HP 0은 보호자 서명 확정 / 치료 접근권 담보 고정으로 잠근다.
...
## 5. 3~7번 기존 후보 산출물의 출시 후 보류 기준

PATCH-02를 legacy_code와 기능 산출물로 보존하고 리오를 8번 후보로 보류한 뒤, 3~7번 기존 후보 산출물 선정은 완료되었다. 다만 이들은 출시 초기 해금 대상이 아니며 HOLD/RETURN/METER/QUIET는 출시 후 보류다.
...
4 강하람 / HOLD-04 / R02 의료·보험 / 기능 산출물 보존 / 출시 후 보류
5 한이루 / RETURN-05 / R03 물류·반품 / 반송 수취인 / 기능 산출물 보존 / 출시 후 보류
6 차유건 / METER-06 / R04 정품·충전·산업 / 폐충전소 계량자 / 기능 산출물 보존 / 출시 후 보류
7 백여울 / QUIET-07 / R08 비상보급·대피 / 침묵권 피난자 / 기능 산출물 보존 / 출시 후 보류
```
...
```text
OPEN-HOST는 3번 산출 번호를 보존하되 NPC 선등장을 유지하고 출시 초기 U5 전환은 금지한다.
3~7번에는 새 로봇을 넣지 않는다.
R04는 폐충전소 계량자로 열어 PATCH/리오와 로봇 과밀을 피한다.
4번 퇴원 보류 환자는 강하람/HOLD-04 프로필 v1.0 기능 산출물로 보존한다.
5번, 6번, 7번 이름/코드명은 프로필 v1.0 기능 산출물로 보존하되 출시 초기 해금으로 쓰지 않는다.
...
9번 Mika Arakawa / GATE-09 상세 결재는 조건부 통과했다.
...
10번 Nora Weiss / BROAD-10 프로필 v1.0은 작성 완료했다.
...
13번 전태주 / GUARD-13 상세 결재 전 후보 재검토는 조건부 유지 판정으로 완료했다.
13번 전태주 / GUARD-13 상세 결재는 조건부 통과 판정으로 완료했다.
13번 전태주 / GUARD-13 조건부 해소는 통과로 승격 가능 판정으로 완료했다.
13번 전태주 / GUARD-13 프로필 v1.0은 작성 완료했다.
15번 Maya Chen / MODEL-15 상세 결재는 조건부 통과 판정으로 완료했다.
15번 MODEL-15 조건부 해소는 통과로 승격 가능 판정으로 완료했다.
15번 Maya Chen / MODEL-15는 E10_C05 개방 후 후보로 보존한다.
17번 Omar Al-Hadid / LOCKOUT-17 산출물은 기능 초안/보류 산출물로만 취급한다.
17번 LOCKOUT-17은 캠페인 미개방으로 대기한다.
Leila Haddad / NAME-18 상세 결재로 넘어가지 않는다.
...
```text
3~7번 기존 후보 산출물을 출시 초기 해금처럼 당기면 R01의 힘이 흐려진다.
출시 초기 캐릭터는 새 대륙보다, 윤서 단독 U5와 첫 보급소/첫 캠페인을 깊게 만드는 쪽이 좋다.
...
```text
1. 출시 초기 플레이어블 포인터 보정 완료 검수를 진행한다.
2. 1~13번 프로필 산출물은 보존하되 출시/해금 확정처럼 쓰지 않는다.
3. 리오 8번 슬롯은 유지하되 직접 해금은 중반 체류·숙박권까지 보류한다.
4. 9~13번은 후보군 선정 또는 상세 결재, 조건부 해소, 프로필 v1.0 기능 산출물 작성이 완료된 상태다. MODEL-15는 E10_C05 개방 후 후보로 보존하고 LOCKOUT-17은 기능 초안/보류 산출물로만 취급한다.
...
8. E01 보조 NPC 최소안은 완료되었으므로, NPC 트랙은 E02 첫 외부 슬라이스 NPC 예산으로 넘어간다.
```
