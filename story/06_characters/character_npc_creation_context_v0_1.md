...
HP 0은 정품 복구/객실 초기화/리콜 직전 인양으로 차별화한다.
...
다음 상세화 대상은 리오 / RIO-08 조건부 해소 문서다.
```
...
```text
1. 리오 / RIO-08 조건부 해소 문서를 작성한다.
2. 6번 폐충전소 계량자가 전기 마법사/엔지니어/충전 버퍼로 납작해지지 않게 프로필 v1.0 잠금을 유지한다.
3. 플레이어블 20인 이름 레지스트리 v0.1은 5번 한이루/RETURN-05, 6번 차유건/METER-06, 7번 백여울/QUIET-07을 L5 확정으로 갱신했다.
4. 반송 수취인은 윤서의 반품 도장과 겹치지 않게 송장/포장끈/반송 스티커 중심으로 유지한다.
5. 7번 침묵권 피난자가 은신자/정찰꾼/생존주의자가 되지 않게 낮은 수신·약한 인양·보급 압력 축을 유지한다.
6. 3번 OPEN-HOST는 상세 프로필보다 R01 NPC 선등장 후 전환 조건과 기능 보존을 점검한다.
7. 리오 8번 슬롯은 유지하되 직접 해금은 중반 체류·숙박권까지 보류한다.
