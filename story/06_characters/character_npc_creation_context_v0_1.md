...
다음 작업은 R01 지역 문서 본문 정리를 판단하는 것이다.
```
...
R01 지역 문서 본문 정리.
산출물 후보:
story/03_regions/r01_region_doc_body_revision_v0_1.md
...
```text
1. R01 NPC/지역 후속 작업은 R01 지역 문서 본문 정리를 먼저 판단한다.
2. 그 다음 플레이어블 20인 로스터 설계도로 돌아간다.
3. 윤서 1번, PATCH 2번 조건부 후보, 리오 8번 후보를 잠금 슬롯으로 둔다.
4. OPEN-HOST는 R01 NPC 선등장 후 플레이어블 전환 후보로 반영한다.
5. 3~7번은 E01/R01/R02/R03/R04/R08 안에서 다시 찾는다.
6. 로스터 중복 검사를 한다.
7. PATCH가 2번째 후보로 충분한지 별도 판정한다.
8. 그 다음 3번째 주인공 후보를 상세 결재한다.
9. 별도로 보급소 NPC/지역 NPC 로스터를 만든다.
10. NPC도 한 명씩 결재한다. R01 NPC 12명 상세화, 본명/호칭 확정, 선택 분기별 후폭풍, 보스 전후 NPC 상태 변화, PORCH-LINE / ROOM-12 재방문 분기, R01 P1 NPC 선택 분기 맵, R01 상태값 통합 매트릭스, R01 NPC 대사 은행, R01 NPC 데이터 스키마 초안, R01 Vertical Slice NPC 구현 범위 재정리, R01 보스 문서와 NPC 연결, R01 보스 문서 본문 정리는 한 바퀴 완료되었고, 다음은 `story/03_regions/r01_remaining_work_list_v0_1.md` 기준으로 R01 지역 문서 본문 정리부터 판단한다.
```
