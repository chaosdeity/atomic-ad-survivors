...
5번 이름/코드명은 프로필 v1.0으로 고정했고, 6~7번 이름/코드명은 playable_20_name_registry_v0_1.md의 L3 후보로 둔다.
5번 반송 수취인 상세 결재는 조건부 통과했다.
5번 반송 수취인 보완 문서에서 통과로 승격 가능 판정을 받았다.
5번 반송 수취인 프로필 v1.0은 한이루/RETURN-05로 작성 완료했다.
다음 상세화 대상은 6번 차유건 / METER-06 / 폐충전소 계량자 상세 결재다.
```
...
```text
1. 6번 차유건 / METER-06 / 폐충전소 계량자 상세 결재를 작성한다.
2. 6번 폐충전소 계량자가 기술자 직업 슬롯으로 납작해지지 않는지 검증한다.
