...
5번과 6번 이름/코드명은 프로필 v1.0으로 고정했고, 7번 이름/코드명은 playable_20_name_registry_v0_1.md의 L3 후보로 둔다.
...
다음 상세화 대상은 7번 백여울 / QUIET-07 / 침묵권 피난자 상세 결재다.
```
...
```text
1. 7번 백여울 / QUIET-07 / 침묵권 피난자 상세 결재를 작성한다.
2. 6번 폐충전소 계량자가 전기 마법사/엔지니어/충전 버퍼로 납작해지지 않게 프로필 v1.0 잠금을 유지한다.
3. 플레이어블 20인 이름 레지스트리 v0.1은 5번 한이루/RETURN-05와 6번 차유건/METER-06을 L5 확정으로 갱신했다. 이후 7번 프로필 작성 때 이름/코드명을 확정 또는 수정한다.
