...
5번 이름/코드명은 프로필 v1.0으로 고정했고, 6번 이름/코드명은 상세 결재 조건부 통과 상태로 L3 후보 유지, 7번 이름/코드명은 playable_20_name_registry_v0_1.md의 L3 후보로 둔다.
...
다음 상세화 대상은 6번 차유건 / METER-06 / 폐충전소 계량자 조건부 해소 문서다.
```
...
```text
1. 6번 차유건 / METER-06 / 폐충전소 계량자 조건부 해소 문서를 작성한다.
2. 6번 폐충전소 계량자가 기술자 직업 슬롯이나 충전 버퍼로 납작해지지 않는지 검증한다.
3. 6번 폐충전소 계량자는 상세 결재에서 조건부 통과했으므로, 전기 마법사/엔지니어/충전 버퍼화 방지 보완 문서를 먼저 작성한다.
4. 플레이어블 20인 이름 레지스트리 v0.1은 5번 한이루/RETURN-05를 L5 확정으로 갱신했다. 이후 6~7번 프로필 작성 때 이름/코드명을 확정 또는 수정한다.
5. 반송 수취인은 윤서의 반품 도장과 겹치지 않게 송장/포장끈/반송 스티커 중심으로 유지한다.
6. 7번 침묵권 피난자가 저항군/탱커가 되지 않게 보급소·인양 안정도 축을 고정한다.
7. 3번 OPEN-HOST는 상세 프로필보다 R01 NPC 선등장 후 전환 조건과 기능 보존을 점검한다.
8. 리오 8번 보류 잠금은 유지한다.
9. 그 다음 시즌 1 큰 줄거리 잠금으로 넘어간다.
10. 별도로 보급소 NPC/지역 NPC 로스터를 만든다.
```
