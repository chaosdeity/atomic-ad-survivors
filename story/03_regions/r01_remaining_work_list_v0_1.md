...
```text
다음 작업.
R01 상태값 NPC 반응 통합 매트릭스까지 완료되었으므로, 상태값 낮음/중간/높음과 조합 위험을 NPC별 대사 조건으로 전환한다.
```
...
```text
R01 보스 전후 NPC 상태 변화 정리, PORCH-LINE / ROOM-12 재방문 분기, R01 P1 NPC 선택 분기 맵, R01 상태값 NPC 반응 통합 매트릭스는 완료되었다.
이제 상태값 낮음/중간/높음과 조합 위험을 NPC별 첫 만남/보스 전후/선택 후/이름 조각 해금 후 대사로 전환해야 한다.
```

그 다음:

```text
2. R01 NPC 데이터 스키마 초안
3. Vertical Slice 구현 우선순위 재정리
4. 플레이어블 20인 로스터 설계도
```
...
이제 그 상태값 조건을 실제 NPC 대사 은행으로 옮길 단계다.
```
