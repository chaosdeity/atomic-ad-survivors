...
R01 Vertical Slice NPC 구현 목록 v0.1도 작성 완료.
R01 NPC 데이터 스키마 적용표 v0.1도 작성 완료.
R01 보스 문서와 NPC 연결 v0.1도 작성 완료.
R01 보스 문서 본문 정리도 완료.
R01 지역 문서 본문 정리도 완료.
R01 전체 문서 인덱스/정리도 완료.
```
...
24. R01 Vertical Slice NPC 구현 목록 v0.1 작성 완료.
25. R01 NPC 데이터 스키마 적용표 v0.1 작성 완료.
26. R01 보스 문서와 NPC 연결 v0.1 작성 완료.
27. R01 보스 문서 본문 정리 완료.
28. R01 지역 문서 본문 정리 완료.
29. R01 전체 문서 인덱스/정리 완료.
```

결론:

```text
R01 캠페인 1 NPC 로스터 12명은 통과 추천 상태와 구현 전달표까지 정리되었다.
이제 R01 안에서 새 NPC를 더 늘리지 않고, E01 다른 지역의 보조 NPC 최소안으로 넘어간다.
```
...
R01_VERTICAL_SLICE_NPC_IMPLEMENTATION_LIST_V0_1.md 작성 완료.
Vertical Slice에 실제로 넣을 NPC, 흔적, 플래그, 상태값, 대사 참조의 최소 범위를 추렸다.
그 다음 씬/오브젝트/대사 수/P1 보류 기준을 구현 목록으로 다시 잘랐다.
```
...
R01_VERTICAL_SLICE_NPC_IMPLEMENTATION_LIST_V0_1.md
```
...
```text
완료.
R01_BOSS_NPC_LINKS_V0_1.md 작성 완료.
첫 보스 전/후 장면에 필요한 NPC 연결을 보스 문서 기준으로 맞췄다.
r01_boss.md는 기존 미추적 초안이므로 이번 작업에서는 읽기만 했다.
```
...
```text
완료.
story/03_regions/r01_boss.md 본문 정리 완료.
스마일 홈 가족심사 관리자 기준, NPC 연결, 보스 패턴별 플래그, 보스 후 FRAME-LEFT 잔향, Vertical Slice 구현 범위를 반영했다.
```

해야 할 일:

```text
1. r01_boss.md를 수정할지 먼저 판단.
2. "스마일 홈 가족심사 관리자" 기준으로 명칭/역할 정리.
3. 보스 패턴별 NPC 플래그 연결 반영.
4. 결절 처리 후 선택을 상태값/NPC 후폭풍과 연결.
5. 죽이는 적보다 절차 중단/결절 처리/기억 추출 언어로 정리.
6. r01_suburb.md는 별도 요청 전까지 건드리지 않음.
```
...
```text
완료.
story/03_regions/r01_suburb.md 본문 정리 완료.
R01을 서부 스마일홈 / 가족·주거·오픈하우스 캠페인 지역으로 고정하고, 9개 구역, 상태값/태그, P0/보스/P1 시간축, Vertical Slice 구현 범위를 반영했다.
```

해야 할 일:

```text
1. r01_suburb.md를 수정할지 먼저 판단.
2. R01 지역 본문이 보스 후 상태값, 재방문, NPC 흔적과 충돌하지 않는지 확인.
3. 보스 문서의 가족심사 관리자 기준과 지역 문서의 오픈하우스/주거/가족 상품화 기준을 맞춘다.
4. Vertical Slice에서 직접 구현할 지역 요소와 후속 보류 요소를 분리한다.
5. 플레이어블 20인 로스터로 넘어가기 전에 R01 지역 기준을 잠근다.
```
...
### 3.6 R01 전체 문서 인덱스/정리

상태:

```text
완료.
story/03_regions/r01_document_index_v0_1.md 작성 완료.
R01 최신 기준 문서, 읽는 순서, 보조/이전 참고 문서, 금지 확장, 다음 작업 포인터를 정리했다.
```

해야 할 일:

```text
1. R01 관련 핵심 문서를 목적별로 묶는다.
2. NPC 설계, 선택 분기, 상태값, 대사, 데이터 스키마, Vertical Slice, 보스, 지역 본문을 찾기 쉽게 정리한다.
3. r01_suburb.md와 r01_boss.md처럼 본문 역할을 하는 문서와 매트릭스/참조 문서를 분리한다.
4. 플레이어블 20인 로스터 설계도로 넘어가기 전 필요한 R01 기준 문서 목록을 잠근다.
```
...
7. 시즌 1 큰 줄거리 잠금으로 넘어가기 전 플레이어블 축과 지역 축의 충돌 확인.
```
...
```text
1. E01 R02/R03/R04/R08 보조 NPC 최소안 완료
...
```text
R01 보스 전후 NPC 상태 변화 정리, PORCH-LINE / ROOM-12 재방문 분기, R01 P1 NPC 선택 분기 맵, R01 상태값 NPC 반응 통합 매트릭스, R01 NPC 대사 은행, R01 NPC 데이터 스키마 초안, R01 Vertical Slice NPC 구현 범위 재정리, R01 Vertical Slice NPC 구현 목록, R01 NPC 데이터 스키마 적용표, R01 보스 문서와 NPC 연결, R01 보스 문서 본문 정리, R01 지역 문서 본문 정리, R01 전체 문서 인덱스/정리는 완료되었다.
이제 R01을 더 확장하지 말고, E01의 다른 지역 보조 NPC를 플레이어블 4~7번과 충돌하지 않게 최소 수량으로 자른다.
```

그 다음:

```text
2. E02 첫 외부 슬라이스 NPC 예산
3. 시즌 1 큰 줄거리 잠금
```
...
R01 Vertical Slice NPC 구현 목록도 NPC별 구현 등급, 첫 노출 위치, 씬/오브젝트, 대사 수, 플래그, 보스 전후 연결, P1 보류 기준으로 정리되었다.
R01 NPC 데이터 스키마 적용표도 npc_id, 구현 묶음, 최소 플래그, 대사 참조, 이름 정책을 묶는 방식으로 정리되었다.
R01 보스 문서와 NPC 연결도 보스 패턴/선택/후폭풍에 NPC 플래그를 붙이는 방식으로 정리되었다.
R01 보스 문서 본문도 최신 기준으로 정리되었다.
R01 지역 문서 본문도 최신 NPC/보스/Vertical Slice 기준으로 정리되었다.
R01 전체 문서 인덱스/정리도 완료되었다.
E01 R02/R03/R04/R08 보조 NPC 최소안도 완료되었다.
이제 E02 첫 외부 슬라이스 NPC 예산으로 넘어갈 단계다.
```
