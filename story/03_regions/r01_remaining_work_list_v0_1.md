...
```text
다음 작업.
R01 보스 전후 시간축이 고정되었으므로, ROOM-12 우회 배급 이후 PORCH-LINE의 줄 변화와 P1 재방문 대사를 분리해 정리한다.
```
...
이제 ROOM-12 우회 배급 이후 PORCH-LINE의 줄이 어떻게 바뀌는지,
그리고 "자리"와 "줄"이 P1 재방문에서 어떻게 서로를 흔드는지 분기 단위로 고정해야 한다.
```

그 다음:

```text
2. R01 P1 NPC 선택 분기 정리
3. R01 상태값 통합 매트릭스
4. R01 NPC 대사 은행
5. R01 NPC 데이터 스키마 초안
6. Vertical Slice 구현 우선순위 재정리
7. 플레이어블 20인 로스터 설계도
```
...
이제 ROOM-12의 자리와 PORCH-LINE의 줄이 재방문에서 어떻게 재배치되는지 분리할 단계다.
```
