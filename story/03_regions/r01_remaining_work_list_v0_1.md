...
이제 새 NPC를 더 늘리기보다, 완성된 R01 문서 묶음을 한 번에 찾을 수 있는 인덱스로 정리해야 한다.
```
...
```text
다음 작업.
R01 NPC, 상태값, 보스, 지역 본문 문서가 많아졌으므로 다음 작업 전에 전체 문서 인덱스를 만든다.
```
...
```text
story/03_regions/r01_doc_index_v0_1.md
```
...
```text
R01 보스 전후 NPC 상태 변화 정리, PORCH-LINE / ROOM-12 재방문 분기, R01 P1 NPC 선택 분기 맵, R01 상태값 NPC 반응 통합 매트릭스, R01 NPC 대사 은행, R01 NPC 데이터 스키마 초안, R01 Vertical Slice NPC 구현 범위 재정리, R01 보스 문서와 NPC 연결, R01 보스 문서 본문 정리, R01 지역 문서 본문 정리는 완료되었다.
이제 플레이어블 20인 로스터로 넘어가기 전에 R01 문서 묶음의 용도와 읽는 순서를 인덱스로 잠가야 한다.
```
