...
```text
다음 작업.
R01 보스 문서 본문까지 정리되었으므로,
R01 지역 본문과 r01_suburb.md를 최신 NPC/보스/Vertical Slice 기준에 맞출지 판단한다.
```
...
또는 사용자가 승인하면 story/03_regions/r01_suburb.md 본문 정리
```
...
```text
R01 보스 전후 NPC 상태 변화 정리, PORCH-LINE / ROOM-12 재방문 분기, R01 P1 NPC 선택 분기 맵, R01 상태값 NPC 반응 통합 매트릭스, R01 NPC 대사 은행, R01 NPC 데이터 스키마 초안, R01 Vertical Slice NPC 구현 범위 재정리, R01 보스 문서와 NPC 연결, R01 보스 문서 본문 정리는 완료되었다.
이제 R01 지역 문서 본문이 최신 보스/NPC/Vertical Slice 기준과 충돌하지 않는지 정리해야 한다.
```
