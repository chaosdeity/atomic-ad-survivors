...
```text
이미 이름 레지스트리 v0.1이 생겼다.
본명/호칭을 먼저 잠그면 이후 대사 은행, 선택 분기, 복희 이름 보관함, FRAME-LEFT 이름 조각이 흔들리지 않는다.
```

그 다음:

```text
2. R01 NPC 선택 분기별 후폭풍 정리
3. R01 보스 전후 NPC 상태 변화 정리
4. PORCH-LINE / ROOM-12 재방문 분기
5. R01 P1 NPC 선택 분기 정리
6. R01 상태값 통합 매트릭스
7. R01 NPC 대사 은행
8. R01 NPC 데이터 스키마 초안
9. Vertical Slice 구현 우선순위 재정리
10. 플레이어블 20인 로스터 설계도
```
