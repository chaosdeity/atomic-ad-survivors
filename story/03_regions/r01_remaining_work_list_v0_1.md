...
```text
다음 작업.
R01 보스 문서와 NPC 연결까지 완료되었으므로,
기존 r01_boss.md 초안에 최신 NPC 연결/플래그/상태값을 반영할지 판단하고 본문을 정리한다.
```
...
```text
R01 보스 전후 NPC 상태 변화 정리, PORCH-LINE / ROOM-12 재방문 분기, R01 P1 NPC 선택 분기 맵, R01 상태값 NPC 반응 통합 매트릭스, R01 NPC 대사 은행, R01 NPC 데이터 스키마 초안, R01 Vertical Slice NPC 구현 범위 재정리, R01 보스 문서와 NPC 연결은 완료되었다.
이제 기존 r01_boss.md 초안을 최신 NPC 플래그와 Vertical Slice 범위에 맞춰 정리해야 한다.
```
