...
```text
다음 작업.
PORCH-LINE / ROOM-12 재방문 분기까지 완료되었으므로, P1 재방문에서 열리는 DRAIN-MARK / SIGN-BACK / KEY-RING / VAC-0 / WELCOME-DESK / FRAME-LEFT의 선택 분기 순서를 정리한다.
```
...
2. 침묵권, 가짜 귀환로, 낡은 관리실, 상담 데스크, 가족사진 프레임의 순서를 정리.
3. 통행태그/수신태그/인양 안정도 상태값에 따른 루트 개폐를 정리.
...
```text
1. R01 P1 NPC 선택 분기 정리
```

이유:

```text
R01 보스 전후 NPC 상태 변화 정리와 PORCH-LINE / ROOM-12 재방문 분기는 완료되었다.
이제 P1 재방문에서 DRAIN-MARK, SIGN-BACK, KEY-RING, VAC-0, WELCOME-DESK, FRAME-LEFT가 어떤 순서와 상태값 조건으로 열리는지 정리해야 한다.
```
...
2. R01 상태값 통합 매트릭스
3. R01 NPC 대사 은행
4. R01 NPC 데이터 스키마 초안
5. Vertical Slice 구현 우선순위 재정리
6. 플레이어블 20인 로스터 설계도
```
...
이제 P1 NPC 선택 분기를 통행태그/수신태그/인양 안정도 조건으로 정리할 단계다.
```
