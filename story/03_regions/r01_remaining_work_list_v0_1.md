...
```text
R01 NPC 본명/호칭 최종 확정은 완료되었다.
이제 각 NPC의 선택 분기와 그 후폭풍을 묶어야, 본명 해금과 상태값 변화가 실제 플레이 흐름에 붙는다.
```
...
3. PORCH-LINE / ROOM-12 재방문 분기
4. R01 P1 NPC 선택 분기 정리
5. R01 상태값 통합 매트릭스
6. R01 NPC 대사 은행
7. R01 NPC 데이터 스키마 초안
8. Vertical Slice 구현 우선순위 재정리
9. 플레이어블 20인 로스터 설계도
```
...
```text
역할명으로 만난 사람들이 언제 이름으로 돌아오는지,
그리고 그 선택이 R01의 상태값과 보스 후폭풍에 어떻게 남는지를 정리하는 단계다.
```
