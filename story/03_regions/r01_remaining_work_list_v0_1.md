...
```text
R01 NPC 선택 분기별 후폭풍 정리는 완료되었다.
이제 그 선택 결과가 첫 보스 전/직후/재방문 시점에 어떤 NPC 상태로 보이는지 시간순으로 고정해야 한다.
```
...
4. R01 상태값 통합 매트릭스
5. R01 NPC 대사 은행
6. R01 NPC 데이터 스키마 초안
7. Vertical Slice 구현 우선순위 재정리
8. 플레이어블 20인 로스터 설계도
```
...
이제 그 결과를 보스 전후 시간표와 재방문 상태 변화로 옮기는 단계다.
```
