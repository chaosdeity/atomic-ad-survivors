...
```text
다음 작업.
R01 P1 NPC 선택 분기 맵까지 완료되었으므로, 인간 안정도/캠페인 농도/흔적 보존/로봇 자율성/보급 압력/수신태그/통행태그/거주태그/인양 안정도를 R01 NPC 12명 반응과 연결한다.
```
...
```text
R01 보스 전후 NPC 상태 변화 정리, PORCH-LINE / ROOM-12 재방문 분기, R01 P1 NPC 선택 분기 맵은 완료되었다.
이제 각 선택 플래그가 인간 안정도, 캠페인 농도, 흔적 보존, 로봇 자율성, 보급 압력, 수신태그, 통행태그, 거주태그, 인양 안정도에 어떤 누적 효과를 만드는지 통합해야 한다.
```
...
이제 R01 상태값을 NPC 12명 반응과 한 표로 묶을 단계다.
```
