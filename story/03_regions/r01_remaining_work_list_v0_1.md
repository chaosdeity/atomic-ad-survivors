...
```text
다음 작업.
R01 NPC 대사 은행까지 완료되었으므로, NPC 기본 프로필, 상태값 반응, 선택 플래그, 이름 해금, 대사 조건을 구현 데이터 스키마로 묶는다.
```
...
```text
1. R01 NPC 데이터 스키마 초안
```

이유:

```text
R01 보스 전후 NPC 상태 변화 정리, PORCH-LINE / ROOM-12 재방문 분기, R01 P1 NPC 선택 분기 맵, R01 상태값 NPC 반응 통합 매트릭스, R01 NPC 대사 은행은 완료되었다.
이제 NPC 기본 프로필, 상태값, 선택 플래그, 이름 해금, 대사 조건을 구현 데이터 스키마로 묶어야 한다.
```

그 다음:

```text
2. Vertical Slice 구현 우선순위 재정리
3. 플레이어블 20인 로스터 설계도
```
