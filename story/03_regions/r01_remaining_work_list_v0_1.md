...
25. R01 보스 문서와 NPC 연결 v0.1 작성 완료.
26. R01 보스 문서 본문 정리 완료.
27. R01 지역 문서 본문 정리 완료.
28. R01 전체 문서 인덱스/정리 완료.
```
...
이제 새 NPC를 더 늘리기보다, 플레이어블 20인 로스터 설계도로 넘어가야 한다.
```
...
```text
R01 보스 전후 NPC 상태 변화 정리, PORCH-LINE / ROOM-12 재방문 분기, R01 P1 NPC 선택 분기 맵, R01 상태값 NPC 반응 통합 매트릭스, R01 NPC 대사 은행, R01 NPC 데이터 스키마 초안, R01 Vertical Slice NPC 구현 범위 재정리, R01 Vertical Slice NPC 구현 목록, R01 보스 문서와 NPC 연결, R01 보스 문서 본문 정리, R01 지역 문서 본문 정리, R01 전체 문서 인덱스/정리는 완료되었다.
이제 R01을 더 확장하지 말고, 플레이어블 20인 로스터에서 윤서/PATCH/OPEN-HOST/리오와 신규 후보들의 역할 충돌을 검토해야 한다.
```
