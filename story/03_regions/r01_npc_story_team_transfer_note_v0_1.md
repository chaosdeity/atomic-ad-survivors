...
24. 다음 작업이 R01 NPC 대사 은행으로 명확히 넘어가는가?
```
...
```text
NPC 12명의 첫 만남, 상태값 낮음/중간/높음, 보스 전후,
선택 후, 이름 조각 해금 후, 보급소/복희/세븐/PATCH 연결 대사를 정리한다.

핵심은 상태값 이름을 직접 설명하게 하는 것이 아니라,
각 NPC의 물건, 자리, 줄, 문패, 주소, 청소 대상, 표식, 비콘, 열쇠,
식탁, 질문, 사진으로 상태값을 드러내는 것이다.
```
