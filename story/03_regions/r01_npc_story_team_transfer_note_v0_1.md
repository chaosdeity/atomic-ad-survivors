...
25. 다음 작업이 R01 NPC 데이터 스키마 초안으로 명확히 넘어가는가?
```
...
```text
NPC 기본 프로필, 상태값 낮음/중간/높음, 선택 플래그,
이름 해금/차단, 대사 조건을 구현 데이터 스키마로 묶는다.

핵심은 문서에 흩어진 NPC 설정을 다시 설명하는 것이 아니라,
이벤트/상태값/대사 출력 시스템이 참조할 수 있는 필드와 키를 고정하는 것이다.
```
