...
31. 다음 작업이 R01 전체 문서 인덱스/정리로 명확히 넘어가는가?
```
...
```text
R01_DOC_INDEX_V0_1.md
```

목표:

```text
R01 관련 핵심 문서의 역할과 읽는 순서를 인덱스로 정리한다.

핵심은 NPC 설계, 선택 분기, 상태값, 대사, 데이터 스키마,
Vertical Slice, 보스, 지역 본문 문서를 목적별로 묶고,
플레이어블 20인 로스터 설계도로 넘어가기 전 참조 기준을 잠그는 것이다.
```
