...
바로 다음 작업 포인터는 **플레이어블 20인 로스터 설계도**로 넘긴다.

이유:

- R01 NPC 12명은 이미 상세화되었다.
- R01 지역/보스/상태값/대사/데이터/Vertical Slice 기준도 정리되었다.
- 이제 개별 지역 NPC를 더 늘리기보다, 플레이어블 후보가 NPC 기능을 빼앗지 않는지 큰 로스터에서 검토해야 한다.
