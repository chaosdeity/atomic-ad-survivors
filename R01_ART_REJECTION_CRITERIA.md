...
R01 아트는 예쁘기만 하면 실패할 수 있다. 승인 기준은 `보기 좋은가`보다 `스마일홈 수장 AI의 현재 상태가 환경으로 읽히는가`다.
...
즉 보존형 관리, 광고 최적화, 라이프라인 잔존은 모두 있어야 하지만 중심은 `빈 가족 칸을 채우려는 집`이어야 한다.
...
| R01-RJ-03 | 스팀펑크/황동/가스마스크 | 프로젝트 톤과 충돌 | 가족 슬롯 결원 복구형 주거 캠페인 톤 + 작은 위험 신호 |
...
| R01-RJ-20 | 수장 AI 성향 부재 | 오브젝트 모음처럼 보여 환경 자체가 캠페인이 되지 않음 | 모든 주요 구조물이 같은 의사결정 기준을 반복하게 재구성 |
| R01-RJ-21 | 스마트 행정 키오스크화 | R01이 주거/가족 캠페인이 아니라 등록 사무소처럼 보임 | 집, 식탁, 문턱, 문패, 창문이 인터페이스가 되게 함 |
| R01-RJ-22 | 사람 노동자화 | 로봇/집이 할 일을 사람이 하고 있어 세계관과 충돌 | 사람은 가족대표/세대원/수령인/보류자 같은 역할 앵커로 제한 |
| R01-RJ-23 | 중앙 광장 군중화 | 세대 단위 분산 보존 구조가 사라짐 | 각 세대/방/문턱/배수로에 분산 배치 |
| R01-RJ-24 | 멸망 대원인 구조 부재 | 스마일홈이 너무 무해한 관리 AI처럼 보여 자유 인간 사회 붕괴 원인이 사라짐 | 라이프라인 의존, 공공 권리층 붕괴, 생명유지망 붕괴, 등록 전쟁/약탈, 회색 오염을 구조로 추가 |
| R01-RJ-25 | 가족 슬롯 결원 복구 부재 | 스마일홈이 다른 관리/라이프라인 AI와 구분되지 않음 | 빈 식탁 자리, 점선 가족사진, 대표자/수령인/보호자 칸, 결원 보정선을 화면 구조로 넣음 |
| R01-RJ-26 | 이유 없는 크림/민트/코랄 팔레트 | 색이 AI 상태가 아니라 예쁜 브랜드 톤으로만 보임 | 오프화이트=빈 집 피부, 앰버=승인 가족 내부, 코랄=결원 수정, 민트=홈케어 손으로 역할 분리 |
| R01-RJ-27 | 청록/녹색 UI 지배 | 스마트 행정/라이프라인 인프라 AI처럼 보임 | 청록/남색 UI는 3% 수준의 계산/인증 빛으로 제한 |
| R01-RJ-28 | 반복 노동을 공장/청소/서비스 노동자로 표현 | 현재 재화가 절차 수행 시간이라는 락을 물리 노동 착취로 오해함 | 체크인 키오스크, 상담 대기선, 갱신 도장, 입주민 앱 알림, 가족 구성 확인표, 보호자 서명 패널, 행복 점수 리뷰 UI로 전환 |
| R01-RJ-29 | 월세 독촉장/빚쟁이/추심 사무소 중심 공포 | 사고 전~중간기 경제 흔적이 현재 핵심 재화처럼 보임 | 돈/미납/추심은 배경 흔적으로 제한하고, 현재 화면은 출석·확인·갱신 루프를 보여줌 |
...
- 집, 택배함, 키오스크, 로봇, 표지판이 각각 따로 노는 오브젝트 모음처럼 보임.
- 스마일홈이 무엇을 선호하고 무엇을 오류로 보는지 화면에서 읽히지 않음.
- R01 14년 가족 복구 변형 운용이 누적됐다기보다, 폐허에 소품을 얹은 것처럼 보임.

수정:

- 모든 주요 구조물이 `집을 기능하게 함`, `가족 구성을 완성함`, `미해결 거주 상태를 줄임`, `흔적을 역할 데이터로 읽음` 중 하나를 수행하게 한다.
- 현관/문턱/식탁/문패/창문/홈케어 장치가 같은 판단을 반복하도록 배치한다.
- 오브젝트 수를 늘리기보다, 한 화면 안의 판단 방향을 통일한다.

### 7.9 사람 배치 오류

반려:

- 사람들이 중앙 광장이나 공용 식당에 모여 있음.
- 사람이 상담원, 경비원, 서비스 직원, 청소 노동자로 기능함.
- 로봇과 집이 할 수 있는 일을 사람이 대신 처리함.

수정:

- 사람은 각 세대/방/문턱/식탁/배수로 가장자리에 분산한다.
- 사람의 기능은 노동이 아니라 가족대표, 세대원, 수령인, 보류자, 이름 보존자, 침묵권 생존자 같은 역할 앵커로 제한한다.
- 사람이 많아야 할 때도 군중이 아니라 서로 분리된 거주 상태로 보여준다.

### 7.10 멸망 대원인 구조 부재

반려:

- 스마일홈이 사람을 잘 관리하는 무해한 주거 AI처럼 보임.
- R01에 라이프라인 의존, 공공 권리층 붕괴, 생명유지망 붕괴, 등록 전쟁, 인간 약탈, 회색 오염이 구조로 보이지 않음.
- 모든 공간이 깨끗하고 따뜻해서 자유 인간 사회가 왜 붕괴했는지 읽히지 않음.

수정:

- 활성 캠페인 구역은 깨끗하게 유지하되, 배수로/문패 아래/식탁 밑/패널 뒤/현관 밖에 잔재를 둔다.
- 시민 안내는 꺼져 있고 입주 보증/가족 승인만 작동하는 대비를 만든다.
- 식탁, 현관, 문, 택배함, 홈케어가 모두 생활 접근 게이트처럼 보이게 한다.
- 찢긴 순번표, 덧쓴 가족 명단, 뜯긴 문패, 보류된 식량태그, 회색 슬러지, 방치된 신발/가방으로 등록 전쟁과 약탈을 고어 없이 보여준다.
